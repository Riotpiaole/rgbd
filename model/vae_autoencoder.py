...
class VAEAutoEncoder(data_model):
    def __init__(
            self,
            title="VAE_auto",
            model_name="vae_autoencoder_batch10",
            img_shape=(
                128,
                128,
                3),
        ...
        self.build()
        self.trianed_weight_path = os.path.join(
            self.weight_path, "vae_en_deencoder.h5")
    ...
    def build(self):
        ...
        with tf.name_scope("Encoder"):
            conv_1 = Conv2D(
                img_chns, kernel_size=(
                    3, 3), strides=(
                    1, 1), padding='same', activation='relu')(x)
            conv_2 = Conv2D(
                filters, kernel_size=(
                    3, 3), padding='same', activation='relu', strides=(
                    2, 2))(conv_1)
            conv_3 = Conv2D(
                filters,
        ...
        with tf.name_scope("Decoder"):
            ...
            decoder_deconv_1 = Conv2DTranspose(
                filters, kernel_size=num_conv, padding='same', strides=(
                    2, 2), activation='relu')(decoder_reshape)
            decoder_deconv_2 = Conv2DTranspose(
                filters, kernel_size=num_conv, padding='same', strides=(
                    2, 2), activation='relu')(decoder_deconv_1)
            decoder_deconv_3_upsamp = Conv2DTranspose(
                filters, kernel_size=(
                    3, 3), strides=(
                    2, 2), padding='same', activation='softmax')(decoder_deconv_2)
            decoder_mean_squash = Conv2D(img_chns, kernel_size=(3, 3), strides=(
                1, 1), padding='same', activation='tanh')(decoder_deconv_3_upsamp)
