...
class GAN(Model):
    def __init__(self,name="generator adverstial net",save_name="GAN",input_shape=(320,320,3),z_dim = 200): 
        ...
        self.total_samples = len(self.data['train'])
    ...
    def train(self):
        disc_noise = np.random.uniform(-1., 1., size=[total_samples, self.z_dim])
        ...
        gen_noise = np.random.uniform(-1., 1., size=[total_samples, self.z_dim])
        ...
        self.model.fit(X_inputs={'input_gen_noise': gen_noise,
                        ...
                        'input_disc_real': self.data['train']},
