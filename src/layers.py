...
def minb_disc(x):
    diffs = K.expand_dims(
        x, 3) - K.expand_dims(K.permute_dimensions(x, [1, 2, 0]), 0)
...
def conv_block_unet(
    ...
    x = LeakyReLU(0.2)(x)
    x = Conv2D(f, (3, 3), strides=strides, name=name,
...
def deconv_block_unet(
    ...
    x = Conv2DTranspose(
        f, (3, 3), output_shape=o_shape, strides=(
            2, 2), padding="same", activation=activation)(x)
    if bn:
        x = BatchNormalization(axis=bn_axis)(x)
    if dropout:
        x = Dropout(0.5)(x)
